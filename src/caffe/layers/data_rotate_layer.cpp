...
void Data_RotateLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void Data_RotateLayer<Dtype>::Forward_cpu(
  ...
  const float radians = caffe_rng_rand() * M_PI * 2.0;
      ...
      //vector< vector< vector<float> > > temp(inputSize, xSize, vector<float>(ySize));
      //vector<vector<vector<int> > > temp(inputSize, vector<vector<float> >(xSize, vector <float>(ySize)));
...
template <typename Dtype>
void Data_RotateLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(Data_RotateLayer);


#endif

INSTANTIATE_CLASS(Data_RotateLayer);
REGISTER_LAYER_CLASS(Data_Rotate);
